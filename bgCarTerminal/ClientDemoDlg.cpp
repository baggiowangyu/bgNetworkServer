...
void CClientDemoDlg::DeviceDisConnect(LLONG lLoginID, char *sDVRIP, LONG nDVRPort)
{
...
{	
	...
	//Enable stream
	BOOL bOpenRet = g_PlayAPI.PLAY_OpenStream(iDispNum,0,0,1024*500);
	if(bOpenRet)
	{
		//Begin play 
...
void CClientDemoDlg::ReceiveRealData(LLONG lRealHandle, DWORD dwDataType, BYTE *pBuffer, DWORD dwBufSize)
{
...
long CClientDemoDlg::GetStreamPort(LLONG lRealHandle)
{
	long lPort=0;
	for(int i=0;i<9;i++)
	{
...
BOOL CClientDemoDlg::SetMultiSpliteMode(int nChannel, int nMultiPlay)
{
	// 获取PrevViewMode
...
void CClientDemoDlg::MultiPlayServerMode(int iDispNum, HWND hWnd)
{
	...
	//Enable stream
	BOOL bOpenRet = g_PlayAPI.PLAY_OpenStream(iDispNum,0,0,1024*100);
	if (!bOpenRet)
	{
		return;
	}
